...
class LaporanController extends Controller
{
    ...
    {
        $and1 = $request->thajaran_id ? "AND t.thajaran_id = '$request->thajaran_id'" : "";
        $and2 = $request->kelas_id ? "AND t.kelas_id = '$request->kelas_id'" : "";
        $and3 = $request->jenis_pembayaran ? "AND t.jenis_pembayaran = '$request->jenis_pembayaran'" : "";
    
        $data = DB::select("
            SELECT p.*, u.nama_lengkap, ta.tahun, jp.pembayaran 
            FROM payment p 
            INNER JOIN users u ON u.id = p.user_id 
            INNER JOIN bulan b ON b.id = p.bulan_id 
            INNER JOIN tagihan t ON t.id = p.tagihan_id 
            INNER JOIN tahun_ajaran ta ON ta.id = t.thajaran_id 
            INNER JOIN jenis_pembayaran jp ON jp.id = t.jenis_pembayaran 
            WHERE 1=1 $and1 $and2 $and3
        ");
